...
def generate_subkey(key):
    ...
    for i in range(16):
        ...
        # Cập nhật left và right cho vòng lặp tiếp theo
        left = left_shift
        right = right_shift
    return subkey
...
def read_wav_file(file_path):
    """Đọc file WAV và trả về các tham số và dữ liệu âm thanh"""
    with wave.open(file_path, 'rb') as wav_file:
        # Lấy các tham số của file WAV
    ...
    return audio_data, n_channels, sample_width, framerate, n_frames

def write_wav_file(file_path, audio_data, n_channels, sample_width, framerate):
    """Ghi dữ liệu âm thanh vào file WAV"""
...
def pad_audio_data(audio_data):
    """Thêm padding vào dữ liệu âm thanh để đảm bảo độ dài là bội số của 8"""
    padding_length = 8 - (len(audio_data) % 8)
    if padding_length == 8:
        padding_length = 0
    padding = bytes([padding_length]) * padding_length
    return audio_data + padding, padding_length

def encrypt_audio(audio_data, key):
    """Mã hóa dữ liệu âm thanh sử dụng DES"""
    # Thêm padding vào dữ liệu âm thanh
    padded_audio, padding_length = pad_audio_data(audio_data)
    
    # Tạo khóa con
    subkey = generate_subkey(key)
    
    # Mã hóa từng khối 64 bit (8 byte)
    encrypted_data = bytearray()
    for i in range(0, len(padded_audio), 8):
        block = padded_audio[i:i+8].decode('latin-1', errors='replace')
        ...
        encrypted_data.extend(encrypted_block.encode('latin-1'))
    
    return bytes(encrypted_data), padding_length

def decrypt_audio(encrypted_data, key, padding_length):
    """Giải mã dữ liệu âm thanh đã mã hóa bằng DES"""
    # Tạo khóa con
    subkey = generate_subkey(key)
    
    # Giải mã từng khối 64 bit (8 byte)
    decrypted_data = bytearray()
    for i in range(0, len(encrypted_data), 8):
        block = encrypted_data[i:i+8].decode('latin-1', errors='replace')
        bit_block = string_to_bit_array(block)
        decrypted_bit_block = des_decrypt_block(bit_block, subkey)
        decrypted_block = bit_array_to_string(decrypted_bit_block)
        decrypted_data.extend(decrypted_block.encode('latin-1'))
...
def main():
    print("CHƯƠNG TRÌNH MÃ HÓA VÀ GIẢI MÃ ÂM THANH BẰNG DES")
    ...
    # Đảm bảo khóa có đủ 8 ký tự
    if len(key) < 8:
        key = key + "0" * (8 - len(key))
    elif len(key) > 8:
        key = key[:8]
    
    print(f"Đang đọc file âm thanh: {input_file}")
    audio_data, n_channels, sample_width, framerate, n_frames = read_wav_file(input_file)
    
    print("Đang mã hóa âm thanh...")
    encrypted_data, padding_length = encrypt_audio(audio_data, key)
    
    # Lưu file đã mã hóa
    encrypted_file = "encrypt.wav"
    write_wav_file(encrypted_file, encrypted_data, n_channels, sample_width, framerate)
    print(f"Đã lưu file âm thanh đã mã hóa: {encrypted_file}")
    
    print("Đang giải mã âm thanh...")
    decrypted_data = decrypt_audio(encrypted_data, key, padding_length)
    
    # Lưu file đã giải mã
    decrypted_file = "decrypt.wav"
    write_wav_file(decrypted_file, decrypted_data, n_channels, sample_width, framerate)
    print(f"Đã lưu file âm thanh đã giải mã: {decrypted_file}")
