...
def write_image(file_path, image_data, height, width, channels):
    """Ghi dữ liệu hình ảnh vào file"""
    # Chuyển mảng byte thành mảng numpy
    image_array = np.frombuffer(image_data, dtype=np.uint8)

    # Reshape mảng thành hình ảnh
...
def pad_image_data(image_data):
    """Thêm padding vào dữ liệu hình ảnh để đảm bảo độ dài là bội số của 8"""
    padding_length = 8 - (len(image_data) % 8)
    if padding_length == 8:
        padding_length = 0
    padding = bytes([padding_length]) * padding_length
    return image_data + padding, padding_length


def encrypt_image(image_data, key):
    """Mã hóa dữ liệu hình ảnh sử dụng DES"""
    # Thêm padding vào dữ liệu hình ảnh
    padded_image, padding_length = pad_image_data(image_data)
    ...
    for i in range(0, len(padded_image), 8):
        block = padded_image[i:i + 8].decode('latin-1', errors='replace')
...
def decrypt_image(encrypted_data, key, padding_length):
    """Giải mã dữ liệu hình ảnh đã mã hóa bằng DES"""
    # Tạo khóa con
    subkey = generate_subkey(key)
